...
# May 11, 2015
# 
# Create Tableau Data Extracts from CSV Data
...
# Try some basic conversion from csv
# use survey data for now
csvFile = r"D:\Survey\HouseholdSurvey2014\data\2014\households.csv"
...
with Extract(tdefile) as extract:
	...
	for line in csvReader:
		...
		# Create the table upon fist row (which may be a header)
		if table is None:
		...
		rowNo += 1
		if rowNo % 100000 == 0:
			print "\b"*32 + locate.format("%d", rowNo, grouping=True), 'rows inserted'

	# terminate progres line
	if rowNo >= 1000000:
